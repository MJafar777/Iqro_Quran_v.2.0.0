export { ListOfSurah } from './ui/ListOfSurah/ListOfSurah';

export { MainHeader } from './ui/MainHeader/MainHeader';

export { MobileAppView } from './ui/MobileApp/MobileAppView';

export type { SearchData } from './model/types/surahType';

export { setSearchReducer } from './model/slice/sliceSearch';

export { getSelectorSearch } from './model/selector/getSelectorSerach';

export { Search } from './ui/Search/Search';
